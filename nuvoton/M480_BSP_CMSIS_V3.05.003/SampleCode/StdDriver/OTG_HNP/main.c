...
void USBOTG_IRQHandler(void)
{
    ...
    {
        printf("[Role Change]\n");
        OTG->INTSTS = OTG_INTSTS_ROLECHGIF_Msk;
    }

    if (reg & en & OTG_INTSTS_GOIDLEIF_Msk)
    {
        printf("[Go Idle] 0x%x\n", OTG->STATUS);
        OTG->INTSTS = OTG_INTSTS_GOIDLEIF_Msk;
        otg_role_change = 0;
    }
    ...
    {
        printf("[VBUS Err]\n");
        OTG->INTSTS = OTG_INTSTS_VBEIF_Msk;
    }

    if (reg & en & OTG_INTSTS_SRPFIF_Msk)
    {
        printf("[SRP Fail]\n");
        OTG->INTSTS = OTG_INTSTS_SRPFIF_Msk;
    }

    if (reg & en & OTG_INTSTS_HNPFIF_Msk)
    {
        printf("[HNP Fail]\n");
        OTG->INTSTS = OTG_INTSTS_HNPFIF_Msk;
        otg_role_change = 0;
    }
    ...
    {
        printf("[SRP Detect]\n");
        OTG->INTSTS = OTG_INTSTS_SRPDETIF_Msk;
        for (i=0; i<10000; i++);
        OTG->CTL |= OTG_CTL_BUSREQ_Msk;
    }
    ...
    {
        printf("[Act as Host]\n");
        OTG->INTEN &= ~OTG_INTEN_HOSTIEN_Msk;
        OTG->INTSTS = OTG_INTSTS_HOSTIF_Msk;
        if (OTG->STATUS & 0x80)
        {
    ...
    if (reg & en & OTG_INTSTS_PDEVIF_Msk)
    {
        ...
        OTG->INTEN &= ~OTG_INTEN_PDEVIEN_Msk;
        OTG->INTSTS = OTG_INTSTS_PDEVIF_Msk;
        if (OTG->STATUS & 0x40)
        {
